...
router.put("/:id/approve", async (req, res) => {
    try {
        const { id } = req.params;
        await pool.query("UPDATE posts SET is_approved = true WHERE id = $1", [
            id,
        ]);
       ...
       WHERE p.id = $1`,
        ...
        if (approvedPost.images && typeof approvedPost.images === "string") {
            try {
                approvedPost.images = JSON.parse(approvedPost.images);
            } catch (e) {
                approvedPost.images = [];
            }
        }
        if (approvedPost.feeling && typeof approvedPost.feeling === "string") {
            try {
                approvedPost.feeling = JSON.parse(approvedPost.feeling);
            } catch (e) {
                approvedPost.feeling = null;
            }
        }
        ...
        try {
            ...
            console.error("Socket emit postApproved error:", e);
        }
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        try {
            ...
            console.error("Socket emit postDeleted error:", e);
        }
